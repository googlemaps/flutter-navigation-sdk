 ...
 * Generated class from Pigeon that represents data sent in messages.
 */
data class NavigationCustomNotificationOptionsDto (
 ...
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): NavigationCustomNotificationOptionsDto {
      val channelId = pigeonVar_list[0] as String
      val title = pigeonVar_list[1] as String
      val body = pigeonVar_list[2] as String
      val smallIconResourceName = pigeonVar_list[3] as String?
      val color = pigeonVar_list[4] as Long?
      val resumeAppOnTap = pigeonVar_list[5] as Boolean
      return NavigationCustomNotificationOptionsDto(channelId, title, body, smallIconResourceName, color, resumeAppOnTap)
    }
  }
  fun toList(): List<Any?> {
    return listOf(
  ...
  override fun equals(other: Any?): Boolean {
    if (other !is NavigationCustomNotificationOptionsDto) {
      return false
    }
    if (this === other) {
      return true
...
data class NavigationNotificationOptionsDto (
  ...
  val resumeAppOnTap: Boolean,
  val customNotificationOptions: NavigationCustomNotificationOptionsDto? = null
)
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): NavigationNotificationOptionsDto {
      ...
      val customNotificationOptions = pigeonVar_list[3] as NavigationCustomNotificationOptionsDto?
      return NavigationNotificationOptionsDto(notificationId, defaultMessage, resumeAppOnTap, customNotificationOptions)
    }
  }
  fun toList(): List<Any?> {
    return listOf(
...
private open class messagesPigeonCodec : StandardMessageCodec() {
  override fun readValueOfType(type: Byte, buffer: ByteBuffer): Any? {
    return when (type) {
      ...
      203.toByte() -> {
        return (readValue(buffer) as? List<Any?>)?.let {
          NavigationCustomNotificationOptionsDto.fromList(it)
        }
      }
      204.toByte() -> {
        return (readValue(buffer) as? List<Any?>)?.let {
  ...
  override fun writeValue(stream: ByteArrayOutputStream, value: Any?)   {
    when (value) {
      ...
      is NavigationCustomNotificationOptionsDto -> {
        stream.write(203)
        writeValue(stream, value.toList())
      }
...
interface NavigationSessionApi {
  fun createNavigationSession(abnormalTerminationReportingEnabled: Boolean, behavior: TaskRemovedBehaviorDto, notificationOptions: NavigationNotificationOptionsDto?, callback: (Result<Unit>) -> Unit)
      ...
      run {
        ...
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            ...
            api.createNavigationSession(abnormalTerminationReportingEnabledArg, behaviorArg, notificationOptionsArg) { result: Result<Unit> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(MessagesPigeonUtils.wrapError(error))
              } else {
