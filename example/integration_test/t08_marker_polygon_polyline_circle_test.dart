...
void main() {
  ...
  patrol('Marker tests', (PatrolIntegrationTester $) async {
    void onMarkerClicked(String event) {
      $.log('Marker clicked event: $event.');
    }
    ...
    void onMarkerDrag(String event, LatLng coordinates) {
      $.log('Marker dragged event: $event. Coorinates: $coordinates.');
    }

    /// The events are not tested because there's currently no reliable way to trigger them.
    void onMarkerInfoWindowAction(String event) {
      $.log('Marker dragged event: $event.');
    }
    ...
    const MarkerOptions markerOneOptions = MarkerOptions(
      ...
      infoWindow: InfoWindow(title: 'Helsinki Office', snippet: 'markerOne'),
    );

    // Add marker and save response to [addedMarkersList].
    final List<Marker?> addedMarkersList = await viewController.addMarkers(
      <MarkerOptions>[markerOneOptions],
    );
    ...
    for (final Marker marker in markers) {
      ...
      expect(
        marker.options.infoWindow.title,
        markerOneOptions.infoWindow.title,
      );
      expect(
        marker.options.infoWindow.snippet,
    ...
    final ImageDescriptor customIcon = await registerBitmapImage(
      bitmap: imageBytes,
      imagePixelRatio: 2,
    );
    ...
    final MarkerOptions markerTwoOptions = MarkerOptions(
      ...
      position: const LatLng(
        latitude: 65.01193816057041,
        longitude: 25.46790635614996,
      ),
      rotation: 70,
    ...
    final List<Marker?> updatedMarkersList = await viewController.updateMarkers(
      <Marker>[markerTwo],
    );
    ...
    for (final Marker marker in markers) {
      ...
      expect(
        marker.options.anchor.u,
        closeTo(markerTwoOptions.anchor.u, tolerance),
      );
      expect(
        marker.options.anchor.v,
      ...
      expect(
        marker.options.consumeTapEvents,
        markerTwoOptions.consumeTapEvents,
      );
      expect(
        marker.options.infoWindow.anchor.u,
        closeTo(markerTwoOptions.infoWindow.anchor.u, tolerance),
      );
      expect(
        marker.options.infoWindow.anchor.v,
      ...
      expect(
        marker.options.infoWindow.title,
        markerTwoOptions.infoWindow.title,
      );
    ...
    const MarkerOptions markerThreeOptions = MarkerOptions(
      position: LatLng(
        latitude: 62.25743381335948,
        longitude: 25.779330148583174,
      ),
      infoWindow: InfoWindow(title: 'Jyväskylä', snippet: 'markerThree'),
    );

    /// Test addMarkers() adds markers in correct order.
    final List<Marker?> removeMarkerList = await viewController.addMarkers(
      <MarkerOptions>[markerThreeOptions, markerOneOptions],
    );
    ...
    expect(
      getRemoveMarkerList.last!.options.infoWindow.title,
      'Helsinki Office',
    );
    ...
    expect(
      getRemovedMarkerList.last!.options.infoWindow.title,
      'Helsinki Office',
    );
    ...
    List<Marker?> clearMarkerList = await viewController.addMarkers(
      <MarkerOptions>[markerOneOptions, markerTwoOptions],
    );
    ...
    clearMarkerList = await viewController.addMarkers(<MarkerOptions>[
      markerOneOptions,
      markerTwoOptions,
    ]);
    getClearMarkerList = await viewController.getMarkers();
  ...
  patrol('Test polylines', (PatrolIntegrationTester $) async {
    ...
    void onPolylineClicked(String event) {
      $.log('Polyline clicked event: $event.');
    }
    ...
    await viewController.addPolylines(<PolylineOptions>[
    ...
    expect(
      polylines[0]!.options.points!.first.latitude,
      closeTo(60.186492, 0.01),
    );
    expect(
      polylines[0]!.options.points!.first.longitude,
      closeTo(24.929471, 0.01),
    );
    expect(polylines[0]!.options.points![1].latitude, closeTo(60.286492, 0.01));
    expect(
      polylines[0]!.options.points![1].longitude,
      closeTo(25.929471, 0.01),
    );
    expect(polylines[0]!.options.clickable, true);
    expect(polylines[0]!.options.geodesic, true);
    expect(
      colorToInt(polylines[0]!.options.strokeColor!),
      colorToInt(Colors.red),
    );
    ...
    if (Platform.isIOS) {
      ...
      expect(
        polylines[0]!.options.strokeJointType,
    ...
    expect(
      colorToInt(receivedPolylines[0]!.options.strokeColor!),
    ...
    expect(
      receivedPolylines4[0]!.polylineId,
      receivedPolylines3[1]!.polylineId,
    );
  ...
  patrol('Polygon tests', (PatrolIntegrationTester $) async {
    void onPolygonClicked(String event) {
      /// The events are not tested because there's currently no reliable way to trigger them.
      $.log('Polygon clicked event: $event.');
    }
    ...
    final double minLongitude = points
        .map((LatLng e) => e.longitude)
        .reduce(min);
    final double maxLongitude = points
        .map((LatLng e) => e.longitude)
        .reduce(max);
    ...
    final List<Polygon?> polygons = await viewController.addPolygons(
    ...
    final LatLng updatedTopLeft = LatLng(
      latitude: topLeft.latitude + 1,
      longitude: topLeft.longitude + 1,
    );
    ...
    final List<LatLng> updatedPoints = createSquare(
      updatedTopLeft,
      updatedBottomRight,
    );
    final List<LatLng> updatedHole = createSquare(
      updatedHoleTopLeft,
      updatedHoleBottomRight,
    );
    final List<LatLng> updatedHole2 = createSquare(
      updatedHoleTopLeft2,
      updatedHoleBottomRight2,
    );
    ...
    final Polygon updatedPolygon = polygons.first!.copyWith(
      options: updatedOptions,
    );

    /// Update polygons with new options and check polygon is updated
    /// and not duplicated.
    final List<Polygon?> updatedPolygons = await viewController.updatePolygons(
    ...
    /// and custom options.
    final List<Polygon?> polygons2 = await viewController.addPolygons(
      <PolygonOptions>[updatedOptions],
    );
    ...
    for (final Polygon polygon in polygonList2) {
      ...
      expect(
        colorToInt(polygon.options.fillColor),
        colorToInt(updatedOptions.fillColor),
      );
      expect(polygon.options.geodesic, updatedOptions.geodesic);
      expect(
        colorToInt(polygon.options.strokeColor),
        colorToInt(updatedOptions.strokeColor),
      );
    ...
    final List<Polygon?> polygons3 = await viewController.addPolygons(
      <PolygonOptions>[options],
    );
    ...
    final List<Polygon?> polygons4 = await viewController.addPolygons(
      <PolygonOptions>[updatedOptions, options, options],
    );
    ...
    final List<Polygon?> polygons5 = await viewController.addPolygons(
      <PolygonOptions>[
        updatedOptions,
        options,
        options,
  ...
  patrol('Circle tests', (PatrolIntegrationTester $) async {
    void onCircleClicked(String event) {
      /// The events are not tested because there's currently no reliable way to trigger them.
      $.log('Circle clicked event: $event.');
    }

    /// Get viewController for the test type (navigation map or regular map).
    ///
    $.log('Circle tests, ${mapTypeVariants.currentValue!}');
    ...
    final List<Circle?> circles = await viewController.addCircles(
    ...
    final Circle updatedCircle = circles.first!.copyWith(
      options: updatedOptions,
    );

    /// Update circles with new options and check circle is updated
    /// and not duplicated.
    final List<Circle?> updatedCircles = await viewController.updateCircles(
    ...
    for (final Circle updatedCircle in updatedCircleList) {
      ...
      expect(
        colorToInt(updatedCircle.options.fillColor),
        colorToInt(updatedOptions.fillColor),
      );
      expect(
        colorToInt(updatedCircle.options.strokeColor),
        colorToInt(updatedOptions.strokeColor),
      );
    ...
    /// and custom options.
    final List<Circle?> circles2 = await viewController.addCircles(
      <CircleOptions>[updatedOptions],
    );
    ...
    final List<Circle> circleList2 = <Circle>[circles2[0]!, getCircles[1]!];
    ...
    for (final Circle circle in circleList2) {
      ...
      expect(
        colorToInt(circle.options.fillColor),
        colorToInt(updatedOptions.fillColor),
      );
      expect(circle.options.strokePattern, updatedOptions.strokePattern);
      expect(
        colorToInt(circle.options.strokeColor),
        colorToInt(updatedOptions.strokeColor),
      );
    ...
    final List<Circle?> circles3 = await viewController.addCircles(
      <CircleOptions>[options],
    );
    ...
    final List<Circle?> circles4 = await viewController.addCircles(
      <CircleOptions>[updatedOptions, options, options],
    );
    ...
    final List<Circle?> circles5 = await viewController.addCircles(
      <CircleOptions>[
        updatedOptions,
        options,
        options,
