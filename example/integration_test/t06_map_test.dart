...
void main() {
  ...
  patrol('Test map UI settings', (PatrolIntegrationTester $) async {
    ...
    void onMyLocationButtonClicked(MyLocationButtonClickedEvent event) {
      $.log('My location button clicked event: currently $event');
    }
    ...
    void onMyLocationClicked(MyLocationClickedEvent event) {
      $.log('My location clicked event: currently $event');
    }
    ...
    for (final bool result in results) {
      ...
      expect(
        await controller.isMyLocationEnabled(),
        result,
      ...
      expect(
        await controller.settings.isMyLocationButtonEnabled(),
        result,
  ...
  patrol('Test map padding', (PatrolIntegrationTester $) async {
    /// For some reason the functionality works on Android example app, but it doesn't work
    /// during the testing. Will skip Android testing for now.
    final ControllerCompleter<GoogleMapViewController> viewControllerCompleter =
        ControllerCompleter<GoogleMapViewController>();
