...
import 'google_maps_navigation_mock.dart';
...
void main() {
  ...
  testWidgets('renders Google Maps Navigation', (WidgetTester tester) async {
  ...
  group('Navigation view API', () {
    ...
    testWidgets('Test camera position and modes', (WidgetTester tester) async {
      ...
      final CameraPositionDto positionIn =
          CameraPositionDto(bearing: 20, target: targetIn, tilt: 30, zoom: 2.0);
      when(viewMockApi.getCameraPosition(any)).thenReturn(positionIn);

      // Get camera position

      final CameraPosition positionOut = await controller.getCameraPosition();
      final VerificationResult result =
      ...
      await controller.followMyLocation(perspectiveIn, zoomLevel: zoomLevelIn);
    ...
    testWidgets('Test camera animations', (WidgetTester tester) async {
      ...
      VerificationResult result = verify(viewMockApi
          .animateCameraToCameraPosition(captureAny, captureAny, captureAny));
      ...
      final LatLngBoundsDto boundsOut = result.captured[1] as LatLngBoundsDto;
      ...
      await controller.animateCamera(CameraUpdate.zoomIn(), duration: duration,
      ...
      await controller.animateCamera(CameraUpdate.zoomOut(), duration: duration,
          onFinished: (bool success) {
        expect(success, true);
      });
    ...
    testWidgets('Test camera movement', (WidgetTester tester) async {
      ...
      result = verify(viewMockApi.moveCameraToLatLng(captureAny, captureAny));
      final LatLngDto latLngOut = result.captured[1] as LatLngDto;
      ...
      const double zoomIn = 4.0;
      await controller.moveCamera(CameraUpdate.newLatLngZoom(latLngIn, zoomIn));
      ...
      const double zoomBy = 2.0;
      const Offset focusIn = Offset(3.0, 4.0);
      await controller.moveCamera(CameraUpdate.zoomBy(zoomBy, focus: focusIn));
    ...
    testWidgets('Test map UI elements', (WidgetTester tester) async {
      ...
      verify(viewMockApi.isScrollGesturesEnabledDuringRotateOrZoom(captureAny));
  ...
  group('Navigation session API', () {
    test('Test terms and conditions flows', () async {
      ...
      final VerificationResult result = verify(sessionMockApi
          .showTermsAndConditionsDialog(captureAny, captureAny, captureAny));
    ...
    test('Test navigation session', () async {
      ...
      final VerificationResult backgroundResult =
          verify(sessionMockApi.allowBackgroundLocationUpdates(captureAny));
      ...
      when(sessionMockApi.continueToNextDestination()).thenReturn(waypointIn);
      ...
      if (waypointOut != null) {
        ...
        expect(
            waypointIn.preferSameSideOfRoad, waypointOut.preferSameSideOfRoad);
    ...
    test('Test navigation simulator', () async {
      ...
      final NavigationWaypoint waypointIn = NavigationWaypoint.withLatLngTarget(
        title: 'title',
        target: const LatLng(latitude: 5.0, longitude: 6.0),
      );
      ...
      final NavigationRouteStatus statusOut3 = await GoogleMapsNavigator
          .simulator
          .simulateLocationsAlongNewRouteWithRoutingAndSimulationOptions(
              <NavigationWaypoint>[waypointIn], routingOptionsIn, simOptionsIn);
  ...
  group('Markers', () {
    ...
    test('update marker', () async {
      ...
      expect(marker.options, markersInMessage[0].options.toMarkerOptions());
      expect(marker.options, markersOut[0]!.options);
    });
    ...
    test('clear map', () async {
      ...
      // Verify correct message sent from view api
      final VerificationResult result = verify(viewMockApi.clear(captureAny));
      final int viewId = result.captured[0] as int;
  ...
  group('Polygons', () {
    test('get polygons', () async {
      ...
      // Mock api response
      final PolygonDto messagePolygon =
          PolygonDto(polygonId: 'Polygon_0', options: polygon.options.toDto());
      when(viewMockApi.getPolygons(any))
          .thenAnswer((Invocation _) => <PolygonDto>[messagePolygon]);

      // Get polygons
      final List<Polygon?> polygonsOut =
          await GoogleMapsNavigationPlatform.instance.getPolygons(viewId: 0);
    ...
    test('add polygon', () async {
      ...
      final List<Polygon?> polygonsOut = await GoogleMapsNavigationPlatform
          .instance
          .addPolygons(viewId: 0, polygonOptions: <PolygonOptions>[optionsIn]);
    ...
    test('update polygon', () async {
      ...
      expect(polygon.options, polygonsInMessage[0].options.toPolygonOptions());
      expect(polygon.options, polygonsOut[0]!.options);
    });
